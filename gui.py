...
class UvsimGUI:
    ...
    def check_run(self):
        ...
        for x in self.memory.mem:
            ...
            # Ignore blank or filler values
            if x in (0, "0", "+0", "-0", "+0000", "+000000"):
                can_run = True
                continue

            # Accept any valid signed integer – let Memory/CPU enforce word size
            if s.isdigit():
                can_run = True
                continue
            ...
            can_run = False
            self.log_message(f"{x} : Invalid memory value. Must be a signed integer.")
            break
        if can_run:
            self.cpu.run()
